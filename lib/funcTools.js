...
class ErrorFile {
  constructor(type,value=undefined,msg,path,location){
    this.type=type
    this.value=value
    this.msg=msg
    this.path=path
...
export function validationWithFile(req, validations, newError,oldFile='') {
  const errors = validations.array();
  const errorValidations = new ErrorValidations(errors);
  if(req.body.image && typeof req.body.image === 'string'){
    if(req.body.image !== oldFile){
      const ErrorNoneFile = new ErrorFile(
        'field',undefined,'not exist image','image','body'
      )
      errorValidations.add(ErrorNoneFile);
    }
  }else if(!req.file) {
